...
class Missile():
    ...
    def __init__(self, params, Id):
        ...
        self.monster_id = Id # 怪兽ID
        self.angle = None
        self.last_attacked_time = 0 # 最近一次要被攻击的时间（未来），用于计算攻击时间间隔
        self.attack_times = 0 #被攻击次数
        self.attacked_prob_list = [] #被攻击时的命中概率列表
        self.loss = self.HP # 击杀一只的奖励，默认为HP值
        self.init_quantity = params['type']['quantity']
        self.mtype = params['type']['mtype'] # 怪兽类型
    ...
    def return_latest_state(self):
        state_now = []
        # (7,)
        # state_now.append(self.missileId)
