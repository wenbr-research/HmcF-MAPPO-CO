# ROMA Trainer: PPO with continuous role embeddings + diversity/consistency regularization
...
class ROMA_Trainer():
    """PPO trainer with continuous role embeddings.
    
    Regularizers:
    - Role diversity: maximize std of z across agents → encourage specialization
    - Role consistency: minimize ||z_tp1_hat - z_tp1|| → encourage stable roles
    """
    ...
    def __init__(self, args, policy, device=torch.device("cpu")):
        ...
        # ROMA hyperparams
        self.role_diversity_coef = getattr(args, 'role_entropy_coef', 0.01)
        self.role_consistency_coef = 0.001
    ...
    def ppo_update(self, sample, update_actor=True):
        ...
        train_info['role_diversity'] = 0
        train_info['role_consistency'] = 0
        ...
        # ---- Encode roles for value and actor loss ----
        role_z, _, _ = self.policy.role_encoder(obs_t)
        ...
        # 2. Actor loss (role-conditioned PPO)
        ...
        # 3. Role encoder loss (re-encode to avoid graph reuse issues)
        _, _, role_std2 = self.policy.role_encoder(obs_t)
        role_diversity2 = -role_std2.mean()
        role_consistency2 = (role_std2 - 0.1).clamp(min=0).mean()
        total_role_loss = (self.role_diversity_coef * role_diversity2 +
                           self.role_consistency_coef * role_consistency2)

        self.policy.role_optimizer.zero_grad()
        total_role_loss.backward()
        if self._use_max_grad_norm:
            role_grad_norm = nn.utils.clip_grad_norm_(self.policy.role_encoder.parameters(), self.max_grad_norm)
        else:
            role_grad_norm = get_gard_norm(self.policy.role_encoder.parameters())
        ...
        train_info['role_diversity'] = role_diversity2.item()
        train_info['role_consistency'] = role_consistency2.item()
    ...
    def train(self, buffer, update_actor=True):
        ...
        train_info = {}
        for k in ['value_loss', 'policy_loss', 'dist_entropy', 'role_diversity',
                   'role_consistency', 'actor_grad_norm', 'critic_grad_norm', 'role_grad_norm', 'ratio']:
            train_info[k] = 0
    ...
    def prep_training(self):
        self.policy.role_encoder.train()
        self.policy.actor.train()
        self.policy.critic.train()

    def prep_rollout(self):
        self.policy.role_encoder.eval()
