...
class Runner(object):
    ...
    def __init__(self, config):
        ...
        elif self.algorithm_name == "rode":
            from onpolicy.algorithms.rode.rode_trainer import RODE_Trainer as TrainAlgo
            from onpolicy.algorithms.rode.role_policy import RODE_Policy as Policy
        elif self.algorithm_name == "roma":
            from onpolicy.algorithms.roma.roma_trainer import ROMA_Trainer as TrainAlgo
            from onpolicy.algorithms.roma.roma_policy import ROMA_Policy as Policy
        else:
            from onpolicy.algorithms.r_mappo.r_mappo import R_MAPPO as TrainAlgo
            from onpolicy.algorithms.r_mappo.algorithm.rMAPPOPolicy import R_MAPPOPolicy as Policy

        share_observation_space = self.envs.share_observation_space[0] if self.use_centralized_V else self.envs.observation_space[0]
        # share_observation_space和observation_space的区别是什么
        # share_observation_space是一个list，里面有两个元素，分别是两个agent的observation_space
        # observation_space是一个list，里面有一个元素，是两个agent的observation_space的拼接
        ...
        if self.algorithm_name == "mat" or self.algorithm_name == "mat_dec":
        ...
        elif self.algorithm_name == "happo" or self.algorithm_name == "hatrpo":
            ...
            n_roles = getattr(self.all_args, "n_roles", 3)
            self.policy = Policy(self.all_args, self.envs.observation_space[0], share_observation_space, self.envs.action_space[0], n_roles=n_roles, device = self.device)
        elif self.algorithm_name == "roma":
            self.policy = Policy(self.all_args, self.envs.observation_space[0], share_observation_space, self.envs.action_space[0], device = self.device)
        else:
            self.policy = Policy(self.all_args, self.envs.observation_space[0], share_observation_space, self.envs.action_space[0], device = self.device)
        ...
        elif self.algorithm_name in ("happo", "hatrpo", "rode", "roma"):
    ...
    def save(self, episode=0):
        ...
        if self.algorithm_name in ("mat", "mat_dec", "rode", "roma"):
