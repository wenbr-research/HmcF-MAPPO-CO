...
class TowerDefenseRunner(Runner):
    ...
    def eval(self, total_num_steps):
        ...
        eval_env_infos = {}
        inter_rewards = np.array([[eval_episode_infos[ei][0]['defense_ratio'], eval_episode_infos[ei][0]['reward']] for ei in range(eval_episode_infos.shape[0])])
        ...
        if self.use_wandb:
            ...
            self.writter.add_scalars('eval_average_episode', {'eval_average_episode_defense': np.mean(inter_rewards[:, 0]),
                                                              'eval_average_episode_rewards': np.mean(inter_rewards[:, 1])}, total_num_steps)
