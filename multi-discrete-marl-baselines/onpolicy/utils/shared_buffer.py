    ...
    def __init__(self, args, num_agents, obs_space, cent_obs_space, act_space):
        ...
        if isinstance(act_space, (list, tuple)):
            ...
            self.available_actions = np.ones(
                (self.episode_length + 1, self.n_rollout_threads, num_agents, act_dim), dtype=np.float32)
        elif hasattr(act_space, "__class__") and act_space.__class__.__name__ == "Discrete":
            self.available_actions = np.ones((self.episode_length + 1, self.n_rollout_threads, num_agents, act_space.n),
                                             dtype=np.float32)
        elif hasattr(act_space, "__class__") and act_space.__class__.__name__ == "MultiDiscrete":
            self.available_actions = np.ones(
                (self.episode_length + 1, self.n_rollout_threads, num_agents, act_space.shape), dtype=np.float32)
        else:
